...
for (const renamer of [true, undefined]) {
    test(`should rename sugarss files: set renamer option to ${renamer}`, async t => {
        const metalsmith = Metalsmith(fixtures('sugarss'))
            .source('src')
            .use(
                postcss({
...
for (const renamer of [false, null]) {
    test(`should not rename sugarss files: set renamer option to ${renamer}`, async t => {
        const metalsmith = Metalsmith(fixtures('sugarss'))
            .source('src')
            .use(
                postcss({
