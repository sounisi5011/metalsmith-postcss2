...
for (const { postcssLocalPlugin, testNameSuffix } of localPlugins) {
    test(`should rename sugarss files: not set renamer option ${testNameSuffix}`, async t => {
    ...
    test(`should rename sugarss files with custom renamer ${testNameSuffix}`, async t => {
        const metalsmith = Metalsmith(fixtures('sugarss'))
            .source('src')
            .use(
                postcssLocalPlugin({
