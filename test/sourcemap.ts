...
for (const { postcssLocalPlugin, testNameSuffix } of localPlugins) {
    test(`should generate multi-level SourceMap file ${testNameSuffix}`, async t => {
    ...
    test(`should generate multi-level SourceMap file with multi source files ${testNameSuffix}`, async t => {
        const metalsmith = Metalsmith(fixtures('sass'))
            .source('src')
            .use(
                sass({
            ...
            .use(
                debuggerPlugin(files => {
                    t.is(
                        getSourceMappingURLType(
                            files['import/main.css'].contents,
                        ),
                        'file',
                        'should exists SourceMap file comment by metalsmith-sass',
                    );
    ...
    test(`should not generate multi-level SourceMap file: Manually override the map.prev option ${testNameSuffix}`, async t => {
        const metalsmith = Metalsmith(fixtures('sass'))
            .source('src')
            .use(
                sass({
    ...
    test(`should generate multi-level SourceMap file: previous process generates inline SourceMap ${testNameSuffix}`, async t => {
        const metalsmith = Metalsmith(fixtures('sass'))
            .source('src')
            .use(
                sass({
    ...
    test(`should generate multi-level inline SourceMap ${testNameSuffix}`, async t => {
        const metalsmith = Metalsmith(fixtures('sass'))
            .source('src')
            .use(
                sass({
        ...
        const inlineSourceMapText = readInlineSourceMap(
            files['a.css'].contents,
        );
    ...
    test(`should change SourceMap file location: a.css ${testNameSuffix}`, async t => {
    ...
    test(`should change SourceMap file location: path/b.css ${testNameSuffix}`, async t => {
        const metalsmith = Metalsmith(fixtures('change-source-map-path'))
            .source('src')
            .use(postcssLocalPlugin());
        const files = await processAsync(metalsmith);
    ...
    test(`should change SourceMap file location: path/to/c.css ${testNameSuffix}`, async t => {
        const metalsmith = Metalsmith(fixtures('change-source-map-path'))
            .source('src')
            .use(postcssLocalPlugin());
        const files = await processAsync(metalsmith);
    ...
    test(`should fix SourceMap file location: Metalsmith source absolute path ${testNameSuffix}`, async t => {
    ...
    test(`should fix SourceMap file location: Metalsmith destination absolute path ${testNameSuffix}`, async t => {
        const metalsmith = Metalsmith(fixtures('sourcemap-broken-location'))
            .source('src')
            .use(postcssLocalPlugin());
        const files = await processAsync(metalsmith);
    ...
    test(`should fix SourceMap file location: Same filename ${testNameSuffix}`, async t => {
        const metalsmith = Metalsmith(fixtures('sourcemap-broken-location'))
            .source('src')
            .use(postcssLocalPlugin());
        const files = await processAsync(metalsmith);
